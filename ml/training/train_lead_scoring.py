...
Reads:   Silver (stg_marketing_leads + stg_marketing_deals)
...
import pandas as pd
from features import build_lead_features
...
# Only pre-conversion features (post-conversion cols null for non-converted → leakage)
CAT_COLS = ["origin"]
NUM_COLS = ["first_contact_month", "first_contact_dayofweek"]
TARGET   = "is_converted"
...
def train() -> str:
    setup_mlflow()
    catalog = get_catalog()
    df = build_lead_features(catalog)
    ...
    with mlflow.start_run() as run:
        ...
        report = classification_report(
            y_test, y_pred, target_names=["not_converted", "converted"]
        )
        log.info("\n%s", report)
        report_path = "/tmp/lead_report.txt"
